use tyson::{Arena, MemoryBlock, make, strmake};
...
fn parse_list<'arena>(
    ...
    let mut list: Vec<Instance<'arena>> = Vec::new();
    while !tokens.is_empty() {
        match tokens.pop() {
            ...
            Some(token) => match token {
                Token::Integer(i) => list.push(Instance::Integer(i)),
                Token::Float(f) => list.push(Instance::Double(f)),
                Token::String(s) => {
...
fn main() {
    ...
    for text in &[CODE, JSON, RUBY_OBJ] {
        let mut tokens = Tokenizer::new(text).into_iter().collect::<Vec<_>>();
        tokens.reverse();

        let mut statements = Vec::new();

        while !tokens.is_empty() {
            statements.push(parse_list(&arena, &mut tokens).unwrap());
        }
